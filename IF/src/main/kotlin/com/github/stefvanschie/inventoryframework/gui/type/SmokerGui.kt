 ...
 * @since 0.8.0
 */
class SmokerGui : NamedGui, InventoryBased {
    /**
    ...
    /**
     * Constructs a new smoker gui for the given `plugin`.
     *
     * @param title the title/name of this gui.
    ...
    @Contract(pure = true)
    override fun copy(): SmokerGui {
        val gui: SmokerGui = SmokerGui(getTitleHolder(), super.plugin)
    ...
    companion object {
         ...
         */
        @Contract(pure = true)
        fun load(instance: Any, inputStream: InputStream, plugin: Plugin): SmokerGui? {
            try {
        ...
        fun load(
            ...
            val smokerGui: SmokerGui = SmokerGui(element.getAttribute("title"), plugin)
            ...
            for (index in 0 until childNodes.getLength()) {
                ...
                when (componentElement.getAttribute("name")) {
                    "ingredient" -> component =
                        smokerGui.ingredientComponent

                    "fuel" -> component = smokerGui.fuelComponent
                    "output" -> component = smokerGui.outputComponent
                    "player-inventory" -> component = smokerGui.playerInventoryComponent
         ...
         */
        @Contract(pure = true)
        fun load(instance: Any, inputStream: InputStream): SmokerGui? {
            return load(
