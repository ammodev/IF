...
class HumanEntityCache {
     ...
     */
    private val inventories: MutableMap<HumanEntity, Array<ItemStack?>> = HashMap()
     ...
     */
    fun storeAndClear(humanEntity: HumanEntity) {
        store(humanEntity)

        val inventory: Inventory = humanEntity.inventory
        for (i in 0..35) {
            inventory.clear(i)
        }
    }
     ...
     * @since 0.5.19
     */
    fun restoreAndForget(humanEntity: HumanEntity) {
        restore(humanEntity)
     ...
     * will return an [IllegalStateException]. The items will be added to the inventory in the same way as the
     ...
     * @since 0.6.1
     */
    fun add(humanEntity: HumanEntity, item: ItemStack): Int {
        val items = inventories[humanEntity]

        checkNotNull(items) {
            "The human entity '" + humanEntity.uniqueId.toString() +
        ...
        for (i in items.indices) {
            ...
            val additionalAmount = min(
                (itemStack.maxStackSize - itemStack.amount).toDouble(),
     ...
     * @since 0.4.0
     */
    fun store(humanEntity: HumanEntity) {
        val items = arrayOfNulls<ItemStack>(36)

        for (i in 0..35) {
            items[i] = humanEntity.inventory.getItem(i)
        }

        inventories[humanEntity] = items
    }
     ...
     * @since 0.4.0
     */
    private fun restore(humanEntity: HumanEntity) {
        val items = inventories[humanEntity] ?: return

        for (i in items.indices) {
            humanEntity.inventory.setItem(i, items[i])
        }
    }
    ...
    private fun restoreAll() {
        inventories.keys.forEach(Consumer { humanEntity: HumanEntity -> this.restore(humanEntity) })
    }
     ...
     * @since 0.10.7
     */
    fun contains(humanEntity: HumanEntity): Boolean {
        return inventories.containsKey(humanEntity)
    }
     ...
     * @since 0.4.0
     */
    fun clearCache(humanEntity: HumanEntity) {
